      ...
      >
        ...
        <div className="p-3 md:p-6 border-b border-gray-700 space-y-3 md:space-y-4">
          ...
          <div className="flex items-center justify-center md:justify-start gap-2">
            ...
            {!isCollapsed && <h1 className="text-lg md:text-xl font-bold md:block">EagleNet</h1>}
          </div>

          {/* USER INFO */}
          {!isCollapsed && (
            <div className="flex items-center gap-2 md:gap-3 px-1">
              <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-teal-500 shrink-0 flex items-center justify-center text-white font-semibold text-xs md:text-sm">
                {user?.firstName?.[0]?.toUpperCase() || user?.name?.[0] || "U"}
              </div>
