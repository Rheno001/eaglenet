      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-2xl border border-slate-200/50 shadow-inner">
          <button
            ...
            className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-300 ${activeTab === 'roles' ? 'bg-white text-slate-900 shadow-xl shadow-slate-200' : 'text-slate-400 hover:text-slate-600'
              }`}
          >
            Roles
          </button>
          <button
            ...
            className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-300 ${activeTab === 'permissions' ? 'bg-white text-slate-900 shadow-xl shadow-slate-200' : 'text-slate-400 hover:text-slate-600'
              }`}
          >
            Permissions
          </button>
                  ...
                  <div className="max-h-[350px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                    {permissions.map((perm) => (
                      <button
                        ...
                        className={`w-full text-left p-4 rounded-2xl border transition-all duration-300 flex items-center justify-between group ${selectedPermissions.includes(perm.id)
                            ? "bg-slate-900 border-slate-900 shadow-xl shadow-slate-200"
                            : "bg-slate-50/50 border-slate-100 hover:border-slate-200"
                          }`}
                      >
                        <div className="min-w-0">
                          <p className={`text-[10px] font-black tracking-widest uppercase truncate ${selectedPermissions.includes(perm.id) ? "text-white" : "text-slate-600"
                            }`}>
                            {perm?.resource}: {perm?.action}
                          </p>
                        </div>
                        <div className={`shrink-0 w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all ${selectedPermissions.includes(perm.id) ? "bg-teal-500 border-teal-500" : "border-slate-200"
                          }`}>
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {currentRoles.map((role) => (
                    <div key={role.id} className="group bg-white p-6 rounded-[2rem] border border-slate-100 hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500 relative overflow-hidden h-fit">
                      <div className="flex items-start justify-between mb-4">
                        <div className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl text-slate-900 group-hover:bg-slate-900 group-hover:text-white transition-all duration-500">
                          <ShieldCheck size={20} />
                        </div>
                        <div className="text-right">
                          <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-lg text-[8px] font-black tracking-widest uppercase">Active</span>
                        </div>
                      </div>

                      <h4 className="text-base font-black text-slate-900 mb-3 tracking-tight uppercase group-hover:text-teal-600 transition-colors truncate">
                        {role.name}
                      </h4>

                      <div className="space-y-2">
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Rules</p>
                        <div className="flex flex-wrap gap-1">
                          {(role.permissions || []).slice(0, 3).map((perm, idx) => (
                            <span key={idx} className="text-[8px] font-black text-teal-600 bg-teal-50/50 px-2 py-0.5 rounded-md border border-teal-100 uppercase tracking-tight">
                              {perm?.resource}_{perm?.action}
                            </span>
                          ))}
                          {(role.permissions?.length > 3) && (
                            <span className="text-[8px] font-black text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100">
                              + {role.permissions.length - 3}
                            </span>
                          )}
                        </div>
                  ...
                  <div className="flex items-center justify-between px-2 pt-2">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-2 bg-white border border-slate-100 rounded-xl hover:shadow-md transition-all disabled:opacity-50"
                      >
