...
export default function UserPayments() {
   ...
   const exportToWord = async () => {
      if (!payments || payments.length === 0) {
         Swal.fire("Info", "No transaction records found to export.", "info");
         return;
      }
         ...
         sections: [{
            ...
            children: [
               ...
               new Paragraph({
                  ...
                  children: [
                     new TextRun({
                        text: "PERSONAL SETTLEMENT DOSSIER & FINANCIAL AUDIT",
                        color: "475569",
                        size: 22,
                        bold: true,
                        smallCaps: true
               ...
               new Paragraph({
                  spacing: { before: 300 },
                  children: [
                     new TextRun({ text: "ACCOUNT HOLDER: ", bold: true, size: 24, color: "0f172a" }),
                     new TextRun({ text: `${user?.firstName || "Client"} ${user?.lastName || ""}`.toUpperCase(), size: 24, color: "3b82f6" }),
                  ],
               }),
               new Paragraph({
                  spacing: { after: 200 },
                  children: [
                     new TextRun({ text: "AUDIT DATE: ", bold: true, size: 18, color: "94a3b8" }),
                     new TextRun({ text: new Date().toLocaleDateString(), size: 18, color: "94a3b8" }),
                  ],
               }),
               new Table({
                  ...
                  rows: [
                     new TableRow({
                        ...
                        children: [
                           new TableCell({
                              shading: { fill: "1e293b" },
                              children: [new Paragraph({ children: [new TextRun({ text: "DATE", color: "ffffff", bold: true })], alignment: AlignmentType.CENTER })]
                           }),
                           new TableCell({
                              shading: { fill: "1e293b" },
                              children: [new Paragraph({ children: [new TextRun({ text: "PAYMENT ID", color: "ffffff", bold: true })], alignment: AlignmentType.CENTER })]
                           }),
                           new TableCell({
                              shading: { fill: "1e293b" },
                              children: [new Paragraph({ children: [new TextRun({ text: "SHIPMENT", color: "ffffff", bold: true })], alignment: AlignmentType.CENTER })]
                           }),
                           new TableCell({
                              shading: { fill: "1e293b" },
                              children: [new Paragraph({ children: [new TextRun({ text: "AMOUNT", color: "ffffff", bold: true })], alignment: AlignmentType.CENTER })]
                           }),
                           new TableCell({
                              shading: { fill: "1e293b" },
                              children: [new Paragraph({ children: [new TextRun({ text: "STATUS", color: "ffffff", bold: true })], alignment: AlignmentType.CENTER })]
                           }),
                        ]
                     }),
                     ...payments.map(pay => (
                        new TableRow({
                           children: [
                              new TableCell({ children: [new Paragraph({ text: new Date(pay.createdAt).toLocaleDateString(), alignment: AlignmentType.CENTER })] }),
                              new TableCell({ children: [new Paragraph({ text: pay.paymentId, alignment: AlignmentType.CENTER })] }),
                              new TableCell({ children: [new Paragraph({ text: pay.shipment?.trackingId || "N/A", alignment: AlignmentType.CENTER })] }),
                              new TableCell({ children: [new Paragraph({ text: `₦${parseFloat(pay.amount).toLocaleString()}`, alignment: AlignmentType.CENTER })] }),
                              new TableCell({ children: [new Paragraph({ text: pay.status, alignment: AlignmentType.CENTER })] }),
                           ]
                        })
                     ))
                  ]
               ...
               new Paragraph({
                  text: `This document verifies a total settled volume of ₦${parseFloat(summary.totalPaid || 0).toLocaleString()}. All listed transactions have been processed through verified secure gateways and are recorded in the Eaglenet Logistics master registry.`,
                  size: 22,
                  color: "475569",
                  spacing: { after: 1200 },
               }),
               new Paragraph({
                  ...
                  children: [
                     new TextRun({
                        text: "OFFICIAL TRANSACTION RECORD - EAGLENET SOLUTIONS - SECURITY VERIFIED",
                        italics: true,
                        size: 16,
                        color: "cbd5e1",
                        bold: true
      ...
      saveAs(blob, `Eaglenet_Payment_Dossier_${new Date().getTime()}.docx`);
   };
      ...
      <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
         ...
         <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div>
               ...
               <p className="text-slate-500 font-medium mt-1">Review Payment History</p>
            </div>
                  ...
                  <table className="w-full border-collapse">
                     <thead>
                        <tr className="bg-slate-50 border-b border-slate-100">
                           <th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Transaction Intelligence</th>
                           <th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Settlement</th>
                           <th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Shipment</th>
                           <th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Timestamp</th>
               ...
               <div className="bg-white rounded-[3rem] w-full max-w-5xl overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-300">
                  ...
                  <div className="p-8 bg-slate-900 text-white flex items-center justify-between">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           <h3 className="text-xl font-black uppercase tracking-tight leading-none mb-1">Payment Details</h3>
                           {/* <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Protocol Verified & Settled</p> */}
                        </div>
                     </div>
                  ...
                  <div className="p-10 space-y-10">
                     ...
                     <div className="flex flex-col items-center text-center p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 shadow-inner">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Transaction Value</p>
                     ...
                     <div className="grid grid-cols-2 gap-10">
                        ...
                        <div className="space-y-6">
                           ...
                           <div>
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Execution Date</p>
