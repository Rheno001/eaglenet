    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await fetch("https://eaglenet-eb9x.onrender.com/api/shipments/mine", { headers });
                const result = await response.json();
                ...
                if (result.status === "success") {
                    ...
                    const pending = shipments.filter(s =>
                        ["ORDER_PLACED", "PENDING_CONFIRMATION", "WAITING_TO_BE_SHIPPED"].includes(s.status)
                    ).length;
                    const inTransit = shipments.filter(s =>
                        ["SHIPPED", "AVAILABLE_FOR_PICKUP"].includes(s.status)
                    ).length;
                    ...
                    setStats({
                        ...
                        inTransit,
                        delivered,
                        processing: shipments.filter(s => s.status === "WAITING_TO_BE_SHIPPED").length,
                        totalBookings: shipments.length,
                        delayed: 0
                    });

                    // Set raw shipments for table
